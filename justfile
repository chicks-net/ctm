...
# validate a release tag early so a bad tag (e.g. v1.2.3-rc1) fails before
# build/attest/sign instead of at release-upload, which would leave dangling
# attestations for a tag that never gets release assets
[group('Release')]
release-validate $rel_version:
    #!/usr/bin/env bash
    set -euo pipefail

    # The '$rel_version' export puts the argument in the environment instead of
    # splicing it into this script as text.  Textual substitution of the argument
    # would let a crafted version like 'v1$(touch /tmp/pwned)' expand during
    # word expansion - double quotes in the justfile source are literal text to
    # bash once the payload's own quotes are spliced in, so they don't stop it.
    # shellcheck disable=SC2050  # {{release_version_regex}} is a just variable, expanded before shellcheck sees it
    # shellcheck disable=SC2154  # rel_version is exported by just via the '$' parameter prefix
    if [[ ! "$rel_version" =~ {{release_version_regex}} ]]; then
        echo "{{RED}}Error: Release version must be vN[.N[.N]] (e.g. v1, v0.2, v2.3.4){{NORMAL}}" >&2
        exit 1
    fi
    echo "{{GREEN}}Release version $rel_version is valid{{NORMAL}}"
...
release-hashes:
    ...
    cd dist
    # extglob excludes sign artifacts so re-hashing after 'just release-sign'
    # doesn't inflate the SLSA subject list with .sig/.pem/.bundle files
    shopt -s nullglob extglob
    binaries=(ctm-!(*.sig|*.pem|*.bundle))
    if [[ "${#binaries[@]}" -eq 0 ]]; then
        echo "{{RED}}Error: no ctm-* binaries in dist/, run 'just release-build' first{{NORMAL}}" >&2
        exit 1
    fi
    sha256sum "${binaries[@]}" | base64 | tr -d '\n'
...
release-upload $rel_version:
    ...
    # '$rel_version' env export is injection-safe; see release-validate for why
    # shellcheck disable=SC2050  # {{release_version_regex}} is a just variable, expanded before shellcheck sees it
    # shellcheck disable=SC2154  # rel_version is exported by just via the '$' parameter prefix
    if [[ ! "$rel_version" =~ {{release_version_regex}} ]]; then
        echo "{{RED}}Error: Release version must be vN[.N[.N]] (e.g. v1, v0.2, v2.3.4){{NORMAL}}" >&2
