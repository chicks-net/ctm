...
After installing `pre-commit`, run `pre-commit install` once in your clone
to enable the git hooks for secrets, shell lint, and whitespace hygiene.

## Pre-commit hooks

The repo ships a `.pre-commit-config.yaml` that wires up:

- `gitleaks` - catches committed secrets before they leave your laptop
- `shellcheck` - lints any shell scripts
- `end-of-file-fixer` / `trailing-whitespace` - basic hygiene

These catch the boring stuff before it ever reaches CI, which is a much
nicer feedback loop than waiting on a github actions run.  To run all
hooks over the whole repo without committing, use:
...
Available recipes:
    [Claude Code]
    claude *args             # Launch Claude Code in the repo root (with arguments optionally)
